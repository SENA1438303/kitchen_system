...
require_once 'entidad/Categoria.Entidad.php';
require_once 'model/Categoria.Model.php';
require_once 'entidad/Proveedor.Entidad.php';
...
if(isset($_REQUEST['action']))
{
	...
	{
		case 'actualizar':
			$alm->__SET('Nombre_insumo',                               $_REQUEST['Nombre_insumo']);
            $alm->__SET('Precio_compra',                                  $_REQUEST['Precio_compra']);
            $alm->__SET('fk_idCategoria',                            $_REQUEST['fk_idCategoria']);
            $alm->__SET('fk_idProveedor',                            $_REQUEST['fk_idProveedor']);
			 
			$model->Actualizar($alm);
			header('Location: edit_insu.php');
			break;

		case 'eliminar':
			$model->Eliminar($_REQUEST['idInsumo']);
			header('Location: edit_insu.php');
			break;

		case 'editar':
			$alm = $model->Obtener($_REQUEST['idInsumo']);
			break;
	}
}
                ...
                <div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-body">
                            <div class="row">
                                <div class="col-lg-6">
                                    <form role="form" action="?action=<?php echo $alm->idInsumo > 0 ? 'actualizar' : 'registrar'; ?>" method="POST">
                                    	<input type="hidden" name="idInsumo" value="<?php echo $alm->__GET('idInsumo'); ?>" />
                                        <div class="form-group">
                                            <label>Nombre del insumo</label>
                                            <input class="form-control" name="Nombre_insumo" required="" autocomplete="off" value="<?php echo $alm->__GET('Nombre_insumo'); ?>">
                                        </div> 
                                        <div class="form-group">
                                            <label>Precio de compra</label>
                                            <input class="form-control" name="Precio_compra" required="" autocomplete="off" value="<?php echo $alm->__GET('Precio_compra'); ?>">
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="fk_idCategoria">
			                                    ...
			                                    <?php
			                                        $tcate = new CategoriaModel();
			                                        foreach($tcate->Listar() as $tca):
			                                    ?>
			                                    <option value="<?php echo $tca->__GET('idCategoria') ?>"
			                                    <?php echo $tca->__GET('idCategoria') == $alm->__GET('idCategoria') ? 'selected' : ''?>>
			                                    <?php echo $tca->__GET('Descripcion_categoria') ?></option>
			                                    <?php endforeach; ?>
                                            </select>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="fk_idProveedor">
                                                <option value="0">--Seleccione--</option>
                                                <?php
                                                    $tproove = new ProveedorModel();
                                                    foreach($tproove->Listar() as $tpro):
                                                ?>
                                                <option value="<?php echo $tpro->__GET('idProducto') ?>"
                                                <?php echo $tpro->__GET('idProducto') == $alm->__GET('idProducto') ? 'selected' : ''?>>
                                                <?php echo $tpro->__GET('Nombre_empresa') ?></option>
                                                <?php endforeach; ?>
                                            </select>
                                        </div>
							                            ...
							                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
							                                ...
							                                <?php foreach($model->Listar() as $r): ?>
                                                                <tr>
                                                                    ...
                                                                    <td><?php echo $r->__GET('Precio_compra'); ?></td>
                                                                    <td><?php echo $r->__GET('fk_idCategoria'); ?></td>
                                                                    <td><?php echo $r->__GET('fk_idProveedor'); ?></td>
                                                                    
                                                                    <td>
                                                                        <a href="?action=editar&idInsumo=<?php echo $r->idInsumo; ?>">Editar</a>
                                                                    </td>
                                                                </tr>
                                                            <?php endforeach; ?>
															</tbody>
